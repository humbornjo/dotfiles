...
--
-- This module can also loaded with `local updater = require("user.utils.updater")`
--
...
local git = require "user.utils.git"
...
function M.update_packages()
  ...
  require("user.utils.mason").update_all()
end
...
function M.update_available(opts)
  ...
  opts = require("user.utils").extend_tbl({ remote = "origin" }, opts)
...
function M.update(opts)
  if not opts then opts = astronvim.updater.options end
  opts = require("user.utils").extend_tbl(
  ...
  then
    ...
    local summary = {
      { "user updated successfully to ", "Title" },
      { git.current_version(), "String" },
      { "!\n", "Title" },
      {
        opts.auto_quit and "user will now update plugins and quit.\n\n"
