...
function M.reload(quiet)
  ...
  if not quiet then -- if not quiet, then notify of result
    if success then
      M.notify("user successfully reloaded", vim.log.levels.INFO)
    else
...
function M.get_icon(kind, padding, no_fallback)
  ...
  if not M[icon_pack] then
    M.icons = astronvim.user_opts("icons", require "user.icons.nerd_font")
    M.text_icons = astronvim.user_opts("text_icons", require "user.icons.text")
  end
