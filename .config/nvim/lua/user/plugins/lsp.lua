    ...
    dependencies = {
      ...
      },
      {
        ...
        opts = function(_, opts)
          ...
          opts.handlers[1] = function(server) require("user.utils.lsp").setup(server) end
        end,
        config = require "user.plugins.configs.mason-lspconfig",
      },
    },
    cmd = function(_, cmds) -- HACK: lazy load lspconfig on `:Neoconf` if neoconf is available
      if require("user.utils").is_available "neoconf.nvim" then table.insert(cmds, "Neoconf") end
    end,
    event = "User AstroFile",
    config = require "user.plugins.configs.lspconfig",
  },
  {
    ...
    event = "User AstroFile",
    opts = function() return { on_attach = require("user.utils.lsp").on_attach } end,
  },
  {
